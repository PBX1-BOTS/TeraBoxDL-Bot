...
Join @MohanishX For Updates
[Anime Site](https://anime.mohanishx.link) 
[Music Site](https://music.mohanishx.link)
[IPTv Site](https://iptv.mohanishx.link)
[Movie Site](https://movie.mohanishx.link)"""
    check_if = await is_user_on_chat(bot, "@MohanishX", m.peer_id)
    if not check_if:
        return await m.reply("Please join @MohanishX then send me the link again.")
    check_if = await is_user_on_chat(bot, "@MohanishX", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @MohanishX then send me the link again."
        )
...
async def handle_message(m: Message):
    ...
    if is_spam and m.sender_id not in [5572938538]:
    ...
    if int(data["sizebytes"]) > 524288000 and m.sender_id not in [5572938538]:
        return await hm.edit(
